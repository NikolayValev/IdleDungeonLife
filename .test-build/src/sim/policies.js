...
exports.AggressivePolicy = exports.ConservativePolicy = exports.BaselinePolicy = void 0;
const modifiers_1 = require("../core/modifiers");
...
function withEquippedItem(run, itemInstanceId) {
    const instance = run.inventory.items.find((item) => item.instanceId === itemInstanceId);
    if (!instance)
        return null;
    const itemDef = items_1.ITEM_REGISTRY.get(instance.itemId);
    if (!itemDef)
        return null;
    return {
        ...run,
...
function bestItemAction(save) {
    const run = save.currentRun;
    if (!run?.alive)
        return null;
    const baseScore = scoreStats((0, modifiers_1.computeStats)(run));
    let best = null;
    for (const instance of run.inventory.items) {
        const candidateRun = withEquippedItem(run, instance.instanceId);
        if (!candidateRun)
            continue;
        const candidateScore = scoreStats((0, modifiers_1.computeStats)(candidateRun));
...
function bestTalentAction(save) {
    const run = save.currentRun;
    if (!run?.alive)
        return null;
    const baseScore = scoreStats((0, modifiers_1.computeStats)(run));
    let best = null;
    for (const talent of talents_1.TALENTS) {
        if (run.talents.unlockedNodeIds.includes(talent.id))
            continue;
        if (!talent.prerequisites.every((id) => run.talents.unlockedNodeIds.includes(id))) {
            continue;
        }
        const effectiveCost = talent.costEssence * (0, modifiers_1.computeStats)(run).talentCostMultiplier;
        if (run.resources.essence < effectiveCost)
            continue;
        const candidateRun = {
            ...run,
            talents: {
        ...
        const delta = scoreStats((0, modifiers_1.computeStats)(candidateRun)) - baseScore;
        if (delta <= 0)
            continue;
        if (!best ||
...
function bestJobAction(save) {
    const run = save.currentRun;
    if (!run?.alive)
        return null;
    const bestJob = save.meta.unlockedJobIds.reduce((best, jobId) => {
        const job = jobs_1.JOB_REGISTRY.get(jobId);
        if (!job)
            return best;
    ...
    if (!bestJob || run.currentJobId === bestJob.jobId)
        return null;
    return { type: "ASSIGN_JOB", jobId: bestJob.jobId };
}
function meetsUnlockRequirement(save, requirement) {
    if (!requirement)
        return true;
...
function bestUnlockAction(save) {
    const run = save.currentRun;
    if (!run?.alive)
        return null;
    const unlockableDungeons = dungeons_1.DUNGEONS.filter((dungeon) => {
        if (save.meta.unlockedDungeonIds.includes(dungeon.id))
            return false;
        const cost = dungeon.unlockRequirement?.legacyAsh ?? 0;
        if (save.meta.legacyAsh < cost)
            return false;
    ...
    const unlockableJobs = Array.from(jobs_1.JOB_REGISTRY.values())
        .filter((job) => {
        if (save.meta.unlockedJobIds.includes(job.id))
            return false;
...
function bestDungeonAction(save, nowUnixSec) {
    const run = save.currentRun;
    if (!run?.alive || run.currentDungeon)
        return null;
    const candidates = dungeons_1.DUNGEONS.filter((dungeon) => save.meta.unlockedDungeonIds.includes(dungeon.id) &&
        run.resources.gold >= dungeon.goldCost).map((dungeon) => {
        const score = (0, stats_1.computeDungeonScore)(run, dungeon.tags);
        const outcome = (0, stats_1.resolveDungeonOutcome)(score, dungeon.difficulty);
        const margin = score - dungeon.difficulty;
        const outcomeWeight = outcome === "success" ? 40 : outcome === "partial" ? 12 : -20;
        const value = dungeon.depthIndex * 100 +
            margin * 3 +
            outcomeWeight -
...
class BaselinePolicy {
    decide(save, nowUnixSec) {
        const run = save.currentRun;
        if (!run?.alive)
            return null;
        return (bestUnlockAction(save) ??
            bestJobAction(save) ??
            bestItemAction(save) ??
...
exports.BaselinePolicy = BaselinePolicy;
/**
...
exports.AggressivePolicy = AggressivePolicy;
