...
exports.LEGACY_PERK_REGISTRY = exports.LEGACY_PERKS = void 0;
exports.getPerkPrerequisite = getPerkPrerequisite;
exports.canPurchasePerk = canPurchasePerk;
const perk = (definition) => definition;
exports.LEGACY_PERKS = [
    // ─── Holy Path ───────────────────────────────────────────────────────────────
    perk({
...
exports.LEGACY_PERK_REGISTRY = new Map(exports.LEGACY_PERKS.map((perk) => [perk.id, perk]));
/** Return the rank-1 prerequisite ID for a given perk, if any. */
function getPerkPrerequisite(perk) {
    if (perk.rank === 1)
        return null;
    const lowerRank = (perk.rank - 1);
    const prereq = exports.LEGACY_PERKS.find((p) => p.path === perk.path && p.rank === lowerRank);
    return prereq?.id ?? null;
}
/** Return whether a perk can be purchased given the meta state. */
function canPurchasePerk(perkId, legacyPath, legacyPerks, legacyAsh) {
    const def = exports.LEGACY_PERK_REGISTRY.get(perkId);
    if (!def)
        return false;
    if (def.path !== legacyPath)
        return false;
    if (legacyPerks.includes(perkId))
        return false;
    if (legacyAsh < def.costAsh)
        return false;
    const prereq = getPerkPrerequisite(def);
    if (prereq && !legacyPerks.includes(prereq))
        return false;
    return true;
}
