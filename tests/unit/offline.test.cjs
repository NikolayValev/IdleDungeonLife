...
test("long offline interval is clamped", () => {
  ...
  const reconciled = reconcileOffline(save, 1000 + 10 * 3600);

  assert.equal(reconciled.updatedAtUnixSec, 1000 + 10 * 3600);
  assert.equal(reconciled.currentRun.lifespan.ageSeconds, BALANCE.maxOfflineSec);
});
