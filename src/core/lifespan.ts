 ...
 */
export function calculateStage(vitality: number): BiologicalStage {
  if (vitality > (1 - STAGE_THRESHOLDS.youth) * 100) return "youth";
  if (vitality > (1 - STAGE_THRESHOLDS.prime) * 100) return "prime";
  if (vitality > (1 - STAGE_THRESHOLDS.decline) * 100) return "decline";
  return "terminal";
}
 ...
 */
export function tickLifespan(
  lifespan: LifespanState,
  stats: ComputedStats,
  elapsedSec: number
): { lifespan: LifespanState; died: boolean } {
  const decayPerSec = BASE_VITALITY_DECAY_PER_SEC * stats.vitalityDecayRate;
 ...
 */
export function applyDungeonWear(
  lifespan: LifespanState,
  stats: ComputedStats,
  baseWear: number,
  isBoss: boolean
...
export const BASE_VITALITY_DECAY_PER_SEC = 100 / (30 * 60);
